package pers.cabin.jichu.a2_io.udp;
 ...
 * <p>
 * Created by caip on 2016/12/8.
 */
public class Demo01_Ping {

    static final int DEFAULT_PORT = 7;
    static final int UDP_HEADER = 20 + 8;
    static final int BACKSPACE = 8; //退格得ASCⅡ代码
    ...
    private static void printStats() {
        ...
        if (received > 0) {
            System.out.println("round-trip min/avg/max = " + minRTT + '/' + ((float) totRTT / received + '/' + maxRTT + " ms"));
        }

    }

    private static void pong(int seq, long past) throws IOException {
    ...
    private static void processPong(long present) {
        ...
        if (flood) {
            System.out.println(
                    (inPacket.getLength() + UDP_HEADER) +
                            " byte from" + inPacket.getAddress().getHostName() +
    ...
    private static void ping(int seq, long past) throws IOException {
    ...
    private static void writeLong(long datum, byte[] dst, int offset) {
        writeInt((int) (datum >> 32), dst, offset);
        writeInt((int) (datum), dst, offset + 4);
    }

    private static void writeInt(int datum, byte[] dst, int offset) {
    ...
    private static void parseArgs(String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            if (args[i].startsWith("-")) {
                ...
                port = ((colon > -1) && (colon < args[i].length() - 1) ? Integer.parseInt(args[i].substring(colon + 1)) : DEFAULT_PORT);
            }
        }
        if (host == null) {
            syntaxError();
    ...
    private static void syntaxError() {
        throw new IllegalArgumentException("Ping [-c count] [-i wait ] [-s packetsize] [-f] hostname [:port]");
    }
}
